...
public class CharsetController implements Initializable {
    ...
    @FXML
    public FormField         root;
    @FXML
    public ComboBox<Charset> charsetComboBox;

    @Override
    public void initialize(URL location, ResourceBundle resources) {
        root.titleProperty().bind(I18nUtil.createStringBinding("App.Formatter.Input.Charset.Header.Title"));
        ...
        charsetComboBox.getSelectionModel().selectedItemProperty().addListener(
                (observable, oldValue, newValue) -> InputFormatProperty.getInstance()
                        .setCharset(newValue == null ? StandardCharsets.UTF_8 : newValue));
